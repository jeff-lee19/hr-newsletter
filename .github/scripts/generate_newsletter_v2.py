...
def call_section_generation(section, week_start, week_end):
    prompt = f"""
당신은 HR 편집장입니다.
반드시 최근 기사만 사용해 한국어 JSON만 반환하세요.
...
def call_weekly_summary(section_payloads, week_start, week_end):
    prompt = (
        ...
        "아래 섹션 데이터를 읽고 weekly_brief와 checklist 6개를 담은 JSON만 반환하세요. "
...
def build_html(edition_label, week_start, week_end, weekly_summary, section_payloads):
    ...
    checklist_html = "\n".join(
        f'                    <li style="margin-bottom: 8px;">{escape(item)}</li>' for item in checklist_items
    )
    weekly_brief = weekly_summary.get("weekly_brief") or "이번 주 HR 이슈는 AI 전환, 채용 구조 변화, 노무 리스크 관리가 동시에 중요해졌다는 점으로 요약됩니다."
